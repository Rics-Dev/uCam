...
actual fun PairPinCode(
    ...
    val connectionState by viewModel.connectionState.collectAsStateWithLifecycle()
    var displayPin by remember { mutableStateOf("") }

    LaunchedEffect(Unit) {
        displayPin = generatePin()
    }
    ...
    Column(
        ...
        Text(
            text = when (connectionState) {
                is ConnectionState.Connected -> "Device paired successfully!"
                is ConnectionState.Connecting -> "Connecting to device..."
                is ConnectionState.Disconnected -> "Enter this PIN code on your other device"
                is ConnectionState.Error -> "Connection failed"
            },
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.Center,
        ...
        if (connectionState !is ConnectionState.Connected) {
            ...
            Text(
                text = "This PIN will expire in 5 minutes",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
        } else {
        ...
        when (connectionState) {
            ...
            is ConnectionState.Error -> {
                ...
                Spacer(modifier = Modifier.height(8.dp))
                FilledTonalButton(
                    onClick = { viewModel.setConnectionState(ConnectionState.Disconnected) },
                    colors = ButtonDefaults.filledTonalButtonColors(
                        containerColor = MaterialTheme.colorScheme.errorContainer,
                        contentColor = MaterialTheme.colorScheme.onErrorContainer
                    )
